...
def play_audio(audio_data, sampling_rate):
    ...
    try:
        ...
        print(
            "Error: 'aplay' not found. Please ensure you have ALSA utilities installed."
        )
...
def main():
    ...
    while True:
        ...
        print("\n" + "=" * 60)
        print(
            f"Index: {current_idx + 1}/{len(active_indices)} (Original ID: {real_idx})"
        )
        if search_query:
        ...
        audio_info = item.get(
            "path"
        )  # 'path' is the audio column name in prepare_data.py
        ...
        if hasattr(audio_info, "get_all_samples"):
            try:
                ...
                if (
                    len(audio_array.shape) > 1
                    and audio_array.shape[0] < audio_array.shape[1]
                ):
        ...
        print("-" * 60)
        print(
            "[N]ext | [P]rev | [J]ump | [S]earch | [C]lear Search | [Pl]ay | [E]xport | [Q]uit"
        )
